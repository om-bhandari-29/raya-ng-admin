...
export class GroupItemUpsert extends Base implements OnInit {
  ...
  private async loadItem(): Promise<void> {
    ...
    try {
      const url = this.apiRoutes.item_group.GET_BY_ID.replace('{id}', String(this.dialogData.itemId));
  ...
  async onSubmit(): Promise<void> {
    ...
    try {
      ...
      if (this.isEditMode()) {
        const url = this.apiRoutes.item_group.UPDATE.replace('{id}', String(this.dialogData.itemId));
